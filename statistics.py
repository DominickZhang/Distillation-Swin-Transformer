...
def main():
    ...
    loss_org = np.array([train_info_org['loss'][epoch][0] for epoch in x_axis])
    lr_org = np.array([train_info_org['lr'][epoch][0] for epoch in x_axis])
    x_mine_axis = list(train_info_mine['loss'].keys())
    loss_mine = np.array([0]*72 + [train_info_mine['loss'][epoch][0] for epoch in x_mine_axis])
    ...
    save_plot(x_axis, np.stack([loss_org, loss_mine], axis=1), 'loss.png', transparent=False, marker_color_list=['b-', 'r-'], legend=['loss_org', 'loss_mine'])
